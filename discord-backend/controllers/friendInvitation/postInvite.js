...
const postInvite = async (req, res) => {
    ...
    if (!targetUser) {
        return res.status(404).send(`Friend of ${targetMailAddress} has not been found. Plase check mail address.`)
    }
    ...
    if (usersAlreadyFriends) {
        return res.status(409).send('Friend Already added. Please check firends list');
    }
